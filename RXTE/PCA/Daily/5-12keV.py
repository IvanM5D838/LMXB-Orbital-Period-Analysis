...
#轉成float的形式丟到x,y陣列裡
    ...
    countRateList.append(float(Datalist[7]))
    errorList.append(float(Datalist[8]))
...
# plt.title('log-log diagram(5-12keV,RXTE)')
# plt.show()
...
# plt.hist(b,bins=100)
# plt.xlim(-5,5)
# plt.title("histogram(5-12keV,RXTE)") 
# plt.show()
...
plt.plot(Frequency,Power,'b',label='5-12keV') 
...
plt.title('ls_periodogram spectrum(RXTE)')
plt.legend(loc='best')
plt.show()
...
def dynamic_ls_periodogram(t = list,ct = list,win = int,mov = int):
    ...
    for i in range(0,n):
        ...
        # dps.append(ls_periodogram(t2[i],ct2[i])[1])
    dps = np.rot90(dps)
    tc = tc - 50000
    return tc,dps
...
# plt.colorbar()
# plt.title('dynamic power spectrum(RXTE(5-12keV))')
# plt.show()
...
# plt.errorbar(Phase,Weighted_Average,yerr = Error,fmt = '^',label = 'superorbital',color = 'r')
