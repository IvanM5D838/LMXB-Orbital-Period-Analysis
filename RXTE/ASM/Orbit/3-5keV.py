...
START = 0
DAYS = 6000
RXTE_PERIOD = 1 / 1.40248124062031

def div(timelist,countlist,errorlist,start,days,n):
    time = []
    count = []
    err = []
    for i in range(len(timelist)):
        if days * (n-1) <= timelist[i] - start <= days * n:
...
with open(f"D:/For Institute/NCU/高能天文實驗室/vscode/X2127+119/RXTE/ASM/Orbit/DWD_lc.tsv", "r") as fobj:
    time, count, error= np.loadtxt(fobj, skiprows=1, usecols=(0, 5, 6), unpack=True)

TimeList_clear,CountRateList_clear,ErrorList_clear = data_red(time,count,error)

cut_time, cut_count, cut_error = div(TimeList_clear, CountRateList_clear, ErrorList_clear, TimeList_clear[0], 1000, 1)
...
def LS_power(time, count):
    ...
    frequency = np.linspace(1.40, 1.41, 2000)
...
# plt.ylabel('power')
# plt.annotate('0.713d', xy=(512.256, 34), xytext=(512.256, 37),
...
plt.errorbar(Phase,Weighted_Average,yerr = Error,fmt = '.',color = 'r', label = 'period = 0.713d')
