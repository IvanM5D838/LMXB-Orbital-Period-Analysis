...
ginga_PERIOD =  0.713014 #confirmed
...
with open(f"D:/For Institute/NCU/高能天文實驗室/vscode/X2127+119/ginga/881020222500_merged.tsv", "r") as fobj:
    time, count, error= np.loadtxt(fobj, skiprows=1, usecols=(0, 1, 2), unpack=True) # first

TimeList, CountRateList, ErrorList = data_red(time, count, error)

plt.errorbar(TimeList - 47456, CountRateList, yerr=ErrorList, linestyle='None', fmt = '.')
plt.legend(loc='upper right')
plt.xlabel('MJD - 47456')
plt.ylabel('Count Rate(ct/s)')
plt.title('Light Curve(Ginga/LAC)')
plt.show()
...
# plt.ylabel('power')
# plt.title('LS_power spectrum(ginga)')
# plt.legend(loc = 'best')
# plt.show()  
...
Weighted_Average, Phase, Error = fold_light_curve(CountRateList, TimeList, ErrorList, ginga_PERIOD, num_bins=32)

# plt.errorbar(Phase,Weighted_Average,yerr = Error,fmt = '.', color = 'r')
# plt.title('Folded Light Curve(Ginga/LAC)')
# plt.legend(loc = 'upper right')
# plt.xlabel('Phase')
# plt.ylabel('Count Rate(ct/s)')
# plt.show()  
...
# plt.colorbar()
# plt.title('dynamic power spectrum(ginga)')
# plt.show()
...
# plt.title(f'multi-sinusoidal fitting(ginga)')
# plt.legend()
# plt.show()
...
hist, bin_edges = np.histogram(simulated_phase_sin, bins=8)
...
params, covariance = curve_fit(gaussian, bin_centers, hist, p0=initial_guess, maxfev=1000)
fitted_amp, fitted_mean, fitted_stddev = params

# plt.hist(simulated_phase_sin, bins=32, edgecolor='black')
# plt.plot(x_values, gaussian(x_values, *params), 'r-', lw=2, label='Fitted Gaussian Distribution', alpha=0.5)
# plt.xlabel('phase')
# plt.ylabel('counts/bin')
# plt.xlim(-0.05, 0.05)
# plt.title(f'Monte Carlo simulation(ginga)')
# plt.legend(loc = 'upper right')
# plt.show()
...
# # 保存为 .tsv 文件
# df.to_csv('ginga.tsv', sep='\t', index=False)
...
# # 保存为 .fits 文件
# hdu.writeto('ginga.fits', overwrite=True)