...
# with open(f"D:/For Institute/NCU/高能天文實驗室/vscode/X2127+119/Chandra/acis/Chandra_acis_merged_lc(with background).tsv", "r") as fobj:
#     time, count, error= np.loadtxt(fobj, skiprows=1, usecols=(0, 1, 2), unpack=True)

with fits.open('D:/For Institute/NCU/高能天文實驗室/vscode/X2127+119/Chandra/acis/acis00675_0.3-10keV_BC.lc') as hdul:
    time = hdul[1].data.field('TIME') # 假設資料在第一個擴展HDU中
    count = hdul[1].data.field('COUNT_RATE')
    error = hdul[1].data.field('COUNT_RATE_ERR')

time = MJDREF + time / 86400

TimeList, CountRateList, ErrorList = data_red(time,count,error)

plt.errorbar(TimeList - 51780, CountRateList, yerr=ErrorList, linestyle='none', fmt = '.')
plt.legend(loc='upper right')
plt.xlabel('MJD - 51780')
plt.ylabel('Count Rate(ct/s)')
plt.title('Light Curve(Chandra/ACIS)')
plt.show()
...
def LS_power(time, count):
    ...
    frequency = np.linspace(1, 2, 2000)
...
# Frequency,Power = LS_power(TimeList,CountRateList_clear)  

# plt.plot(Frequency,Power,'b') 
# plt.xlabel('frequency(cycle/year)')
# plt.ylabel('ls_power')
# plt.title('LS_power spectrum(Chandra)')
# plt.legend(loc='best')
# plt.show()
...
# plt.colorbar()
# plt.title('dynamic power spectrum(Chandra)')
# plt.show()
...
Weighted_Average,Phase,Error = fold_light_curve(CountRateList, TimeList, ErrorList, Chandra_PERIOD, num_bins=32)

# plt.errorbar(Phase,Weighted_Average,yerr = Error,fmt = '.',color = 'r')
# plt.title('Folded Light Curve(Chandra/ACIS)')
# plt.legend(loc = 'upper right')
# plt.xlabel('Phase')
# plt.ylabel('Count Rate (ct/s)')
# plt.show()  
...
# plt.xlabel('phase')
# plt.ylabel('count rate(ct/s)')
# plt.title('multi-sinusoidal fitting(Chandra)')
# plt.legend()
# plt.show()
...
def monte_carlo_simulation(count, phase, error):
    ...
    for _ in range(num_simulation):
        ...
        valid_indices = np.where((phase >= 0) & (phase <= 0.5))[0]
...
hist, bin_edges = np.histogram(simulated_phase_sin, bins=32)
...
# plt.xlim(-0.5, 0.5)
# plt.title(f'Monte Carlo simulation(Chandra)')
# plt.legend()
# plt.show()
...
#     # 讀取 FITS 檔案
#     with fits.open(f'D:/For Institute/NCU/高能天文實驗室/vscode/X2127+119/Chandra/hrc/{filename}') as hdul:
...
#         df = pd.DataFrame({'Time': time, 'Count': data['COUNT_RATE'], 'Error': data['COUNT_RATE_ERR']})
...
# # 將 Table 保存為 FITS 文件
# table.write('Chandra_hrc_merged_lc.fits', format='fits', overwrite=True)

# # 儲存合併後的資料
# merged_df.to_csv('Chandra_hrc_merged_lc.tsv', index=False, sep='\t')
