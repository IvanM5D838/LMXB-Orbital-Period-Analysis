...
exosat_PERIOD =  0.713014 #confirmed
...
# with open(f"D:/For Institute/NCU/高能天文實驗室/vscode/X2127+119/exosat/exosat_merged.tsv", "r") as fobj:
#     time, count, error= np.loadtxt(fobj, skiprows=1, usecols=(0, 1, 2), unpack=True)

with fits.open('D:/For Institute/NCU/高能天文實驗室/vscode/X2127+119/exosat/c28831_bc.lc') as hdul:
    time = MJDREF + (hdul[1].header['TSTART'] + hdul[1].data.field('TIME')) / 86400 # 假設資料在第一個擴展HDU中
    count = hdul[1].data.field('RATE')
    error = hdul[1].data.field('ERROR')

TimeList, CountRateList, ErrorList = data_red(time, count, error)

plt.errorbar(TimeList - 46358, CountRateList, yerr=ErrorList, linestyle='none', fmt = '.')
plt.legend(loc='upper right')
plt.xlabel('MJD - 46358')
plt.ylabel('Count Rate(ct/s)')
plt.title('Light Curve(EXOSAT/LE)')
plt.show()
...
# plt.plot(Frequency,Power,label='0.05-2keV') 
# plt.xlabel('frequency(cycle/year)')
# plt.ylabel('power')
# plt.title('LS_power spectrum(exosat)')
# plt.legend(loc = 'best')
# plt.show()  
...
Weighted_Average, Phase, Error = fold_light_curve(CountRateList, TimeList, ErrorList, exosat_PERIOD, num_bins=32)

plt.errorbar(Phase,Weighted_Average, yerr = Error, fmt = '.', color = 'r')
plt.title(f'Folded Light Curve(EXOSAT/LE)')
plt.legend(loc = 'upper right')
plt.xlabel('Phase')
plt.ylabel('Count Rate(ct/s)')
plt.show()  
...
# plt.colorbar()
# plt.title('dynamic power spectrum(exosat)')
# plt.show()
...
def multiple_sinusoidal_model(x, y, err, n):# n = order
    ...
    return model_x, model_y, num_data_points, obs_x, obs_y
...
# plt.title(f'multi-sinusoidal fitting(EXOSAT le)')
# plt.legend()
# plt.show()
...
hist, bin_edges = np.histogram(simulated_phase_sin, bins=64)
...
# plt.xlim(-0.5, 0.5)
# plt.title(f'Monte Carlo simulation(EXOSAT le)')
# plt.legend()
# plt.show()
...
# # 保存为 .tsv 文件
# df.to_csv('exosat_merged.tsv', sep='\t', index=False)
...
# # 保存为 .fits 文件
# hdu.writeto('exosat_merged.fits', overwrite=True)
