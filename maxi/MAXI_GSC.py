...
MAXI_PERIOD =  0.713014 #confirmed
...
MJDREFI = 51544.0007428037037037

# with open(f"D:/For Institute/NCU/高能天文實驗室/vscode/X2127+119/maxi/Orbit_lc.tsv", "r") as fobj:
#     time, count1, error1= np.loadtxt(fobj, skiprows=0, usecols=(0, 3, 4), unpack=True) # 2-4keV

# with open(f"D:/For Institute/NCU/高能天文實驗室/vscode/X2127+119/maxi/Orbit_lc.tsv", "r") as fobj:
#     time, count2, error2= np.loadtxt(fobj, skiprows=0, usecols=(0, 5, 6), unpack=True) # 4-10keV

with fits.open('D:/For Institute/NCU/高能天文實驗室/vscode/X2127+119/maxi/2-10keV_bc.fits') as hdul:
...
START_DAYS = split_data[0][0] - TimeList[0]
END_DAYS = split_data[0][-1] - TimeList[0]

# flare time = 59800-59875
...
plt.title('Light Curve(MAXI/GSC)')
plt.show()
...
def linear_detrend(x, y):
    ...
    # 計算斜率 a 和截距 b
    a = sum((x[i] - mean_x) * (y[i] - mean_y) for i in range(n)) / sum((x[i] - mean_x)**2 for i in range(n))
    b = mean_y - a * mean_x  # 截距

    # 計算趨勢和去趨勢後的數據
    trend = [a * x[i] + b for i in range(n)]
    detrended = [y[i] - trend[i] for i in range(n)]

    return detrended, trend, b, a

detrend_count, trend_count, intercept, slope = linear_detrend(TimeList, CountRateList)

# plt.scatter(TimeList, detrend_count, s=4)
# plt.legend(loc='upper right')
# plt.xlabel('Time(s)')
# plt.ylabel('count rate(ct/s)')
# plt.title('NICER(10341401)')
# plt.show()
...
# plt.plot(Frequency,Power,label='2-4 keV') 
...
Weighted_Average, Phase, Error = fold_light_curve(cut_count, cut_time, cut_error, MAXI_PERIOD, num_bins=32)

plt.errorbar(Phase,Weighted_Average,yerr = Error,fmt = '.', color = 'r')
plt.title(f'Folded Light Curve(MAXI/GSC)({int(split_data[0][0])}-{int(split_data[0][-1])})')
plt.legend(loc = 'upper right')
plt.xlabel('Phase')
plt.ylabel('Count Rate(ct/s)')
plt.show()  
...
# plt.colorbar()
# plt.title('dynamic power spectrum(MAXI(orbit))')
# plt.show()
...
# plt.title(f'multi-sinusoidal fitting(MAXI)({int(cut_time[0])}-{int(cut_time[-1])})')
# plt.legend()
# plt.show()
...
# plt.hist(simulated_phase_sin, bins=128, edgecolor='black')
# plt.plot(x_values, gaussian(x_values, *params), 'r-', lw=2, label='Fitted Gaussian Distribution')
# plt.xlabel('phase')
# plt.ylabel('counts/bin')
# plt.xlim(-0.5, 0.5)
# plt.title(f'Monte Carlo simulation(MAXI GSC)')
# plt.legend()
# plt.show()
...
# MJDREFI = 51544
# MJDREFF = 7.4287037037037 * 10**(-4)

# TimeList = TimeList - MJDREFI - MJDREFF

# # 创建一个Astropy表格
# data_table = Table([TimeList, CountRateList, ErrorList], names=('TIME', 'RATE', 'ERROR'))

# # 将表格保存为FITS文件
# fits_file = "D:/For Institute/NCU/高能天文實驗室/vscode/X2127+119/maxi/2-10keV_new.fits"
# data_table.write(fits_file, format='fits', overwrite=True)
