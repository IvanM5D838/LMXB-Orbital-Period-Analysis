...
heao_1_PERIOD =  0.713014 #confirmed
...
with open(f"D:/For Institute/NCU/高能天文實驗室/vscode/X2127+119/heao-1/heao-1(HED).tsv", "r") as fobj:
    time, count, error= np.loadtxt(fobj, skiprows=1, usecols=(0, 1, 2), unpack=True) 

# with open(f"D:/For Institute/NCU/高能天文實驗室/vscode/X2127+119/heao-1/heao-1(MED).tsv", "r") as fobj:
#     time, count, error= np.loadtxt(fobj, skiprows=1, usecols=(0, 1, 2), unpack=True) 

TimeList, CountRateList, ErrorList = data_red(time, count, error)

plt.errorbar(TimeList - 43000, CountRateList, yerr=ErrorList, linestyle = 'none', fmt = '.')
plt.xlabel('MJD - 43000')
plt.ylabel('Count Rate(ct/s)')
plt.title('Light Curve(HEAO-1/HED)')
plt.show()
...
# plt.plot(Frequency,Power,label='2.5-60keV') 
# plt.xlabel('frequency(cycle/year)')
# plt.ylabel('power')
# plt.title('LS_power spectrum(heao-1)')
# plt.legend(loc = 'best')
# plt.show()  
...
Weighted_Average, Phase, Error = fold_light_curve(CountRateList, TimeList, ErrorList, heao_1_PERIOD, num_bins=32)

# plt.errorbar(Phase,Weighted_Average,yerr = Error,fmt = '.', color = 'r')
# plt.title('Folded Light Curve(HEAO-1/MED)')
...
# plt.colorbar()
# plt.title('dynamic power spectrum(heao-1(orbit))')
# plt.show()
...
# plt.title(f'multi-sinusoidal fitting(heao-1 MED)')
# plt.legend()
# plt.show()
...
x_values = np.linspace(-0.5, 0.5, 2000)
initial_guess = [np.max(hist), np.mean(simulated_phase_sin), standard_deviation]  # 初始猜测的振幅、均值和标准差
...
# plt.xlim(-0.5, 0.5)
# plt.title(f'Monte Carlo simulation(heao-1 MED)')
# plt.legend()
# plt.show()
...
# # 保存为 .tsv 文件
# df.to_csv('heao-1(HED)_merged.tsv', sep='\t', index=False)

# ----------------------------------------------------------------

# # 定义文件名列表
# filenames = [
#     'a2_h3l_1h2128n120_a_bc.lc',
#     'a2_h3l_1h2128n120_b_bc.lc',
#     'a2_h3l_1h2128n120_c_bc.lc',
#     'a2_h3s_1h2128n120_a_bc.lc',
#     'a2_h3s_1h2128n120_b_bc.lc',
#     'a2_h3s_1h2128n120_c_bc.lc'
# ]
...
#         # 假设时间、计数率和误差都在第一个表格（HDU[1]）
#         time = hdul[1].data['TIME'] / 86400 + MJDREF
...
# # 排序时间、计数率和误差
# time = all_time[sorted_indices]
# count = all_rate[sorted_indices]
# error = all_error[sorted_indices]
...
# # 创建 FITS 文件中的表格数据
# col1 = fits.Column(name='TIME', format='E', array=np.array(TimeList))
# col2 = fits.Column(name='RATE', format='E', array=np.array(CountRateList))
# col3 = fits.Column(name='ERROR', format='E', array=np.array(ErrorList))

# # 将列组合成一个表
# hdu = fits.BinTableHDU.from_columns([col1, col2, col3])

# # 保存为 .fits 文件
# hdu.writeto('heao-1(HED)_merged.fits', overwrite=True)
